	...
	<div class="container">
		<div class="content">
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ./views/index-table-order-pent-liquid.php");
			}else{
			...
			if(isset($_POST['liquidate'])){
				...
				if($row['paid']==0){
					...
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se puede liquidar si aun no ha sido cobrada esta venta.</div>';
				
				}else{

				$fechaEdit	     	= date('y/m/d');
				$status				= "Liquidada";
				$usuario			= $_SESSION['inputUserLogin'];

				$update = mysqli_query($connect, "UPDATE sales SET liquidate_for='$usuario', fecha_edit='$fechaEdit', fecha_liquid='$fechaEdit', status='$status' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-liquid.php?nik=".$nik."&pesan=sukses");
				}else{
			...
			<form class="form-horizontal" action="" method="post">
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<input type="submit" name="liquidate" class="btn btn-sm btn-success" value="Liquidar">
						<a href="../views/index-table-order-pent-liquid.php" class="btn btn-sm btn-info">Regresar</a>				
					</div>
				</div>
			</form>
		</div>
