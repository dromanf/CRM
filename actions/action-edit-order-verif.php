	...
	<div class="container">
		<div class="content">
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ./views/index-table-order-pent-verif.php");
			}else{
			...
			if(isset($_POST['verificate'])){
				...
				$noteVerif	     	= mysqli_real_escape_string($connect,(strip_tags($_POST["noteVerif"],ENT_QUOTES)));//Escanpando caracteres 
				$noteVerifcoment	= mysqli_real_escape_string($connect,(strip_tags($_POST["noteVerifcoment"],ENT_QUOTES)));//Escanpando caracteres 
				$fechaEdit	     	= date('y/m/d');
				$fechaEditcoment	= date('y/m/d h:m:s');
				$coment 			= "(".$fechaEditcoment." -- ".$_SESSION['inputUserLogin'].") // ".$noteVerif." /--------------------------/ ".$noteVerifcoment;
				$status				= "Verificada";

				$update = mysqli_query($connect, "UPDATE sales SET agent='$agente', name_client='$nameClient', name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcessClient', card='$cardClient', name_card='$nameCardClient', date_exp_card='$dateExpCardClient', cvc_card='$cvcCardClient', price='$price', re_order='$reOrder', note_agent='$noteAgent', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', fecha_edit='$fechaEdit', fecha_valid='$fechaEdit', note_verif='$coment', status='$status' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-verif.php?nik=".$nik."&pesan=sukses");
				}else{
			...
			if(isset($_POST['modificate'])){
				...
				$update = mysqli_query($connect, "UPDATE sales SET agent='$agente', name_client='$nameClient', name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcessClient', card='$cardClient', name_card='$nameCardClient', date_exp_card='$dateExpCardClient', cvc_card='$cvcCardClient', price='$price', re_order='$reOrder', note_agent='$noteAgent', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', fecha_edit='$fechaEdit', note_verif='$coment' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-verif.php?nik=".$nik."&pesan=sukses");
				}else{
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
			...
			<form class="form-horizontal" action="" method="post">
				...
				<?php if($row ['product1']==""){
					...
					<div class="col-sm-4">
						<input type="text" name="product1" value="'.$row ['product1'].'" class="form-control" placeholder="Producto 1" required="required">
						<input type="text" name="qty1" maxlength="2" value="'.$row ['qty1'].'" class="form-control" placeholder="Cantidad" required="required">
					</div>
				</div>';
				}
				if($row ['product2']==""){
					...
					<div class="col-sm-4">
						<input type="text" name="product1" value="'.$row ['product2'].'" class="form-control" placeholder="Producto 2" required="required">
						<input type="text" name="qty1" maxlength="2" value="'.$row ['qty2'].'" class="form-control" placeholder="Cantidad" required="required">
					</div>
				</div>';
				}
				if($row ['product3']==""){
					...
					<div class="col-sm-4">
						<input type="text" name="product3" value="'.$row ['product3'].'" class="form-control" placeholder="Producto 3" required="required">
						<input type="text" name="qty3" maxlength="2" value="'.$row ['qty3'].'" class="form-control" placeholder="Cantidad" required="required">
					</div>
				</div>';
				}
				if($row ['product4']==""){
					...
					<div class="col-sm-4">
						<input type="text" name="product4" value="'.$row ['product4'].'" class="form-control" placeholder="Producto 4" required="required">
						<input type="text" name="qty4" maxlength="2" value="'.$row ['qty4'].'" class="form-control" placeholder="Cantidad" required="required">
					</div>
				</div>';
				}
				if($row ['product5']==""){
					...
					<div class="col-sm-4">
						<input type="text" name="product5" value="'.$row ['product5'].'" class="form-control" placeholder="Producto 5" required="required">
						<input type="text" name="qty5" maxlength="2" value="'.$row ['qty5'].'" class="form-control" placeholder="Cantidad" required="required">
					</div>
				</div>';
				}				
				?>			
				...
				<?php if($row['note_verif']==""){
					echo '<div class="form-group">
					<label class="col-sm-3 control-label">Comentario del Verificador</label>
					<div class="col-sm-4">
						<textarea name="noteVerif" maxlength="64000" style="height: 100px;" class="form-control" placeholder="Comentario del verificador"></textarea>
						</div>
					</div>';
				}else{
					echo '<div class="form-group">
					<label class="col-sm-3 control-label">Comentario del Verificador</label>
					<div class="col-sm-4">
						<textarea name="noteVerifcoment" maxlength="64000" style="height: 100px;" class="form-control" readonly="readonly">'.$row['note_verif'].'</textarea>
						</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<input type="submit" name="verificate" class="btn btn-sm btn-success" value="Verificar">
						<input type="submit" name="modificate" class="btn btn-sm btn-primary" value="Guardar">						
						<a href="../views/index-table-order-pent-verif.php" class="btn btn-sm btn-info">Regresar</a>				
					</div>
				</div>
			</form>
		</div>
