	...
	<div class="container">
		<div class="content">
			...
			if(mysqli_num_rows($sql) == 0){
				header("Location: ./views/index-table-order-pent-valid.php");
			}else{
			...
			if(isset($_POST['validate'])){
				...
				$noteValid	     	= mysqli_real_escape_string($connect,(strip_tags($_POST["noteValid"],ENT_QUOTES)));//Escanpando caracteres 
				$noteValidcoment	= mysqli_real_escape_string($connect,(strip_tags($_POST["noteValidcoment"],ENT_QUOTES)));//Escanpando caracteres 
				$fechaEdit	     	= date('y/m/d');
				$fechaEditcoment	= date('y/m/d h:m:s');
				$coment 			= "(".$fechaEditcoment." -- ".$_SESSION['inputUserLogin'].") // ".$noteValid." /--------------------------/ ".$noteValidcoment;
				$status				= "Validada";

				$update = mysqli_query($connect, "UPDATE sales SET agent='$agente', name_client='$nameClient', name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcessClient', card='$cardClient', name_card='$nameCardClient', date_exp_card='$dateExpCardClient', cvc_card='$cvcCardClient', price='$price', re_order='$reOrder', note_agent='$noteAgent', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', fecha_edit='$fechaEdit', fecha_valid='$fechaEdit', note_valid='$coment', status='$status' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-valid.php?nik=".$nik."&pesan=sukses");
				}else{
			...
			if(isset($_POST['modificate'])){
				...
				$update = mysqli_query($connect, "UPDATE sales SET agent='$agente', name_client='$nameClient', name_client='$nameClient', lastname_client='$lastNameClient', phone='$phone', phone_alt='$phoneAlt', gender='$genClient', age='$ageClient', address='$direccion', zip_code='$zipcode', zip_4='$zipfour', city='$city', state='$state', payment_type='$payment', procesadora='$cardProcessClient', card='$cardClient', name_card='$nameCardClient', date_exp_card='$dateExpCardClient', cvc_card='$cvcCardClient', price='$price', re_order='$reOrder', note_agent='$noteAgent', product1='$productOne',  qty1='$qtyOne', product2='$productTwo', qty2='$qtyTwo', product3='$productThree', qty3='$qtyThree', product4='$productFour', qty4='$qtyFour', product5='$productFive', qty5='$qtyFive', fecha_edit='$fechaEdit', note_valid='$coment' WHERE id='$nik'") or die(mysqli_error());
				if($update){
					header("Location: action-edit-order-valid.php?nik=".$nik."&pesan=sukses");
				}else{
					echo '<div class="alert alert-danger alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Error, no se pudo guardar los datos.</div>';
			...
			<form class="form-horizontal" action="" method="post">
				...
				<?php if($row['note_valid']==""){
					echo '<div class="form-group">
					<label class="col-sm-3 control-label">Comentario del Validador</label>
					<div class="col-sm-4">
						<textarea name="noteValid" maxlength="64000" style="height: 100px;" class="form-control" placeholder="Comentario del validador"></textarea>
						</div>
					</div>';
				}else{
					echo '<div class="form-group">
					<label class="col-sm-3 control-label">Comentario del Validador</label>
					<div class="col-sm-4">
						<textarea name="noteValidcoment" maxlength="64000" style="height: 100px;" class="form-control" readonly="readonly">'.$row['note_valid'].'</textarea>
						</div>
				...
				<div class="form-group">
					...
					<div class="col-sm-6">
						<input type="submit" name="validate" class="btn btn-sm btn-success" value="Validar">
						<input type="submit" name="modificate" class="btn btn-sm btn-primary" value="Guardar">						
						<a href="../views/index-table-order-pent-valid.php" class="btn btn-sm btn-info">Regresar</a>				
					</div>
				</div>
			</form>
		</div>
